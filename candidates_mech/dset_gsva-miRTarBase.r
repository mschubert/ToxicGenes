...
sys$run({
    ...
    sets = gset$get_human("miRTarBase_2017")
    sets = sets[sapply(sets, function(s) args$gene %in% s)]
    ...
    print(plt$text(sprintf("miRTarBase target expression (%i)", util2$nc(scores)), size=20))
